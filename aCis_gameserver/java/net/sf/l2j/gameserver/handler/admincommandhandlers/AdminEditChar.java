...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.startsWith("admin_setsex"))
		{
			...
			Sex sex = (player.getAppearance().getSex() == Sex.MALE ? Sex.FEMALE : Sex.MALE);
			try
			{
				final StringTokenizer st = new StringTokenizer(command, " ");
				st.nextToken();
			...
			{
				Player character = target.getActingPlayer();
				player.getAppearance().setSex(sex);
				player.sendMessage("Your gender has been changed to " + sex.toString() + " by a GM. You will get disconected in 3 seconds!");
				ThreadPool.schedule(() -> character.logout(false), 3000);
				player.broadcastUserInfo();
