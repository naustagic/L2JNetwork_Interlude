...
public final class RequestJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (target.isInsideZone(ZoneId.FLAG))
		{
			requestor.sendMessage("You cannot request a party in Arena battle!");
			return;
		}
		...
		if ((requestor.isInSoloZone()) || (target.isInSoloZone()))
		{
			if (target.isInSoloZone())
			{
				requestor.sendMessage(target.getName() + " is inside this zone.");
				return;
			}
			if (requestor.isInSoloZone())
			{
				requestor.sendMessage("You can't request party inside this zone.");
				return;
			}
		}
