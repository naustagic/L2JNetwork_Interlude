...
public class CreatureAI extends AbstractAI
{
	...
	protected boolean maybeMoveToPawn(WorldObject target, int offset)
	{
		...
		if (!_actor.isInsideRadius(target, offset, false, false))
		{
			if (getFollowTarget() != null)
			{
				// Use the new moving intersection check?
				if (Config.NEW_AI_MOVING_ATTACK_CHECK)
				{
					if (!_actor.isInsideMovingRadius(target, offset + Config.NEW_AI_MOVING_ATTACK_CHECK_RADIUS))
					{
						return true;
					}
				}
