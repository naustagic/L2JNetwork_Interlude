 ...
 * @author Baggos
 */
public final class SchemeBuffer extends Npc
{
	...
	{
		/*
		 * if(Config.community_board_in_npcs) { CommunityBoard.getInstance().handleCommands(activeChar.getClient(), "_bbsnpcs;openfile;buffer;index"); return; }
		 */
		...
		// html.replace("%intro%", ServerSideImage.getInstance().getRankNameImageHtmlTag(activeChar, 2, 256, 64));
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("dobuff"))
		{
			...
			{
				if (player.getInventory().getInventoryItemCount(Config.VOTE_BUFF_ITEM_ID, 0) >= 1)
				{
					player.getInventory().destroyItemByItemId("VoteCoins", Config.VOTE_BUFF_ITEM_ID, 1, player, null);
			...
			if (votebuff != null)
			{
				if (player.getInventory().getInventoryItemCount(Config.VOTE_BUFF_ITEM_ID, 0) >= Config.VOTE_BUFF_ITEM_COUNT)
				{
					player.getInventory().destroyItemByItemId("VoteCoins", Config.VOTE_BUFF_ITEM_ID, Config.VOTE_BUFF_ITEM_COUNT, player, null);
					player.getInventory().updateDatabase();
					player.sendPacket(new ItemList(player, true));
					player.sendMessage(Config.VOTE_BUFF_ITEM_COUNT + " vote stone destroyed.");
				}
				else
				{
					player.sendMessage("You dont have enough (" + Config.VOTE_BUFF_ITEM_COUNT + ") vote item for buff.");
					return;
				}
			}
	...
	private static int getFee(ArrayList<Integer> list)
	{
		...
		for (int sk : list)
		{
			if (Config.BUFFER_BUFFLIST.get(sk) == null)
				continue;
